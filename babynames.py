...
- Получите текст из файла и выведите его в консоль check
- Сделайте проверку на существование файла. Если файл не существует - 
    ...
    ошибки sys.exit(1)
- Напишите регулярное выражение, выбирающее данные из таблицы, и выведите check
...
- Обработайте полученный список, удалив из каждой строки лишние пробелы 
    ...
    консоль. (Подсказка: для обработки списков удобно использовать функцию map) check
- Преобразуйте полученные данные в словарь заданной структуры и выведите check
...
def extract_names(filename):
    ...
    i = 0
    for el in names:
        to_remove = re.search(r'<td .+\s+', el)
        names[i] = el.strip(to_remove.group())
        i += 1
    g = 0
    for el in data:
        to_remove = re.search(r'\s', el)
        data[g] = el.strip(to_remove.group())
        to_remove = re.search(r'\n', el)
        data[g] = data[g].strip(to_remove.group())
        g += 1
    #print(data)
